...
class Job extends CI_Controller {
    ...
    public function lists()
    {
        ...
        if($data['student']['coop_status_id'] > 1) {
            ...
            foreach($jobs as $row) {
                ...
                $temp['benefit'] = @$this->Company->get_benefit_by_company($row['company_id'])[0];
                if(
    ...
    { //in-s002
        ...
        if($data['student_profile']['Mother_Status'] == "มีชีวิต" ){
            ...
            $data['email_person'] = $this->Job_model->get_email_by_company_id($company_id);
            
            for($x=0;$x<count($data['email_person']);$x++){
                $to = $data['email_person'][$x];
                $subject = 'ระบบสหกิจศึกษา มหาวิทยาลัยบูรพา';
                $msg = 'ขณะนี้มีนิสิตได้สมัครเข้ามาในสถานประกอบการของท่าน ท่านสมารถไปตรวจสอบได้ที่ลิ้ง'. site_url();
                //sentmail here
